...
- Model routing: Route simple queries (scope guard, basic sentiment) to Claude Haiku 4.5 ($0.38/M tokens) vs. GPT-4o ($10/M tokens) — 26× cost reduction on low-complexity tasks.
...
| REVIEWLENS AI
Cost Estimation & SaaS Unit Economics
AI Inference  •  Vector Storage  •  Pricing Tiers  •  Margin Model
Version 1.0  |  March 2026  |  Aiden (Chin Wei Mak) |
| --- |

| Key Principle
Every user action in an AI-native SaaS burns real GPU compute. Flat-rate pricing models are existential risks — a power user on a $20/mo plan can cost $200/mo to serve. The hybrid credit model solves this. |
| --- |
...
| Insight Report generates 3× the expected tokens | Set max_tokens caps on each worker agent. If Worker 1 output > 8K tokens, truncate and log — downstream workers use truncated version. |